...
export const inspectRetrieval=(query:string,knowledgeBaseId?:string):Promise<RetrievalInspect>=>parse<RetrievalInspect>(fetch('/api/retrieval-lab/inspect',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({query,knowledge_base_id:knowledgeBaseId||null,limit:10})}))
...
export const createRetrievalCase=(body:{name:string;question:string;knowledge_base_id:string|null;expected_document_ids:string[];expected_keywords:string[];expected_no_answer:boolean}):Promise<RetrievalCase>=>parse<RetrievalCase>(fetch('/api/retrieval-lab/cases',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}))
export const deleteRetrievalCase=(id:string):Promise<void>=>parse<void>(fetch(`/api/retrieval-lab/cases/${id}`,{method:'DELETE'}))
export const runRetrievalCases=():Promise<RetrievalRun>=>parse<RetrievalRun>(fetch('/api/retrieval-lab/runs',{method:'POST'}))
