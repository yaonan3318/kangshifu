...
export interface BatchRecord { id:string; name:string; category:string|null; tags:string[]; note:string|null; status:BatchStatus; total_count:number; indexed_count:number; processing_count:number; duplicate_count:number; failed_count:number; pending_count:number; created_at:string; updated_at:string; completed_at:string|null }
...
export interface BatchCreate { name:string; category?:string|null; tags?:string[]; note?:string|null; files:{relative_path:string;original_name:string;size_bytes:number}[] }
