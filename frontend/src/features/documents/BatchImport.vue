...
async function submit(){
  ...
  try { const batch=await createBatch({name:name.value.trim(),category:category.value||null,tags:tags.value.split(',').map(x=>x.trim()).filter(Boolean),note:note.value||null,files:files.value.map(file=>({relative_path:pathOf(file),original_name:file.name,size_bytes:file.size}))})
...
<template><section class="upload-panel batch-import">
  ...
  <div class="batch-form"><label>批次名称<input v-model="name" placeholder="例如：2026年8月工资资料"></label><label>资料分类<input v-model="category" placeholder="可选"></label><label>标签<input v-model="tags" placeholder="多个标签用逗号分隔"></label><label>备注<input v-model="note" placeholder="可选"></label></div>
