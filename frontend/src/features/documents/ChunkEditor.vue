...
<template><li :class="{'chunk-disabled':chunk.enabled===false}"><div class="chunk-meta"><strong>片段 {{chunk.sequence_number}}</strong><span v-if="chunk.manually_edited">人工修改</span><span v-if="chunk.token_count">{{chunk.token_count}} 词元</span><span>{{chunk.enabled===false?'已停用':'参与检索'}}</span></div><p v-if="error" class="error">{{error}}</p><textarea v-if="editing" v-model="content" class="chunk-textarea"></textarea><pre v-else>{{chunk.content}}</pre><div class="row-actions"><button @click="editing=!editing">{{editing?'取消':'编辑'}}</button><button v-if="editing" :disabled="busy||!content.trim()" @click="action(()=>updateChunk(chunk.id,content))">保存并重建索引</button><button @click="action(()=>setChunkEnabled(chunk.id,chunk.enabled===false))">{{chunk.enabled===false?'启用':'停用'}}</button><button @click="action(()=>reindexChunk(chunk.id))">重建索引</button><button v-if="chunk.manually_edited" @click="action(()=>restoreChunk(chunk.id))">恢复原文</button></div></li></template>
