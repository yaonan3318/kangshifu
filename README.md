...
- P2-0 质量基线：评测集管理（含 CSV/Excel 导入、期望文档与期望片段）、检索配置版本化，以及新旧版本在同一评测集上的指标对比（旧值/新值/差值/升降/响应时间）。片段召回率按期望 chunk_id 真实命中计算，不用命中文档数冒充。
- P2-1 检索准确性：Query Rewrite 与多轮上下文补全、Multi-query 多查询召回、Reranker 精排（含重排前后名次）、元数据/结构化过滤（问答与检索共用）、管理员维护的中文检索词典（同义词/缩写/专有名词/中英文映射）与拼写纠正；检索日志保存召回与精排前后排名及各阶段耗时。
- P2-2 回答可靠性：答案置信度分级（高/中/资料不足）、无答案与低置信度归因处理、引用真实性校验（编号/真实片段/权限/删除停用/版本/位置）与“推断”标记、可追溯引用（版本变化提示“版本已更新”、命中关键词、下载）、按问题类型的答案结构模板。
- P2-3 助手角色增强：助手可配置答案模板/联网/无答案策略/能力说明与知识库范围；预置六个角色助手。知识库范围语义明确：综合助手可配置全部知识库，专项助手未绑定知识库时不可检索并提示“尚未配置资料范围”，不默认放大权限；未接入联网服务时明确返回“联网服务未配置”。用户端只展示可理解的信息（当前助手、知识库、是否允许通用知识与运维），并提供助手欢迎页。
- P2-4 轻量 Chatflow：固定画布节点真正按图执行（start/classify/context_completion/query_rewrite/multi_query/knowledge_search/reranker/condition/local_model/deepseek/harness/answer_check/final_answer），支持条件分支、节点启停、失败/超时/fallback、逐节点耗时与安全摘要、草稿/发布/回滚版本；调试运行与正式问答共用同一执行引擎，生产只使用已发布流程，未发布时使用安全默认流程。
- P2-5 用户体验与展示：回答过程可视化与流式优化、生成任务恢复（持久化 Answer Job、后台生成不依赖浏览器、刷新/断线按游标续传、仅显式停止才取消、幂等 request_id、服务重启失败恢复）、推荐追问、答案操作（复制/重新生成/导出 MD·PDF/反馈/举报）、知识缺口中心、首页数据看板。
- P2-6 文档理解能力：PDF 标题层级/页眉页脚/多栏/表格/内嵌图片 OCR 与图注关联、扫描页 OCR 与 Office 解析增强；按知识库配置切片策略并预览（预览与索引共用同一套切片函数）；父子切片子片段命中返回父片段；文档作者/部门/主题/生效失效/关联文档/替代版本等图谱基础数据。
